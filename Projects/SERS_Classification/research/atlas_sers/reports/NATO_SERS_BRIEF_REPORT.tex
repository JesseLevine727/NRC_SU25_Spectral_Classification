...
\begin{figure}[H]
\centering
\input{p03_classical/figures/tikz/R1_within_station_random_forest_body.tex}
...
\begin{figure}[H]
\centering
\input{p03_classical/figures/tikz/R4_chemical_confusion_body.tex}
...
The classical benchmark is now the reference. The next modelling step is to train an ordinary compact one-dimensional neural classifier, followed by acquisition-aware versions that make repeated views of one sample agree while separating chemical categories. All models will use the same preprocessing and physical-sample partitions for a direct learning comparison; improved transfer will not prove removal of noise, substrate variation, or background.

Separately, the preprocessing study will compare minimal scaling, smoothing, baseline correction, source-selected platform-family processing, and spectrum-quality routing under identical splits and training-only decisions. The analysis will audit whether gains preserve chemistry rather than instrument or substrate cues. No substrate family passed all classical instrument-independence criteria, so neural results will be limited to supported station--substrate--instrument cells. Results will include spectrum-level and per-sample performance, coverage, and instrument-specific behavior. The question is whether learning or preprocessing helps weak acquisition combinations without sacrificing chemical separation.
