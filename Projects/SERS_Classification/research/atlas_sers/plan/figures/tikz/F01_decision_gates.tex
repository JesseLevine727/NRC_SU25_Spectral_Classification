% DATA_SHA256: f70fdf2d66bf01357b8e4a17d93e2383529dd487aca97d09bfd845fa619dc290
...
\begin{tikzpicture}[node distance=8mm and 13mm]
  ...
  \node[planbox, fill=oiBlue!12, below=15mm of g4] (routeb) {Route B: classical benchmark\\or Route C: failure study};
  ...
  \node[gate, below=18mm of d0] (gf) {GF\\Figure parity};
  \node[planbox, fill=oiPurple!16, right=15mm of gf] (freeze) {Publication freeze\\native TikZ + standalone HTML};
  \draw[flow] (gf) -- node[above,labeltext]{pass} (freeze);
  \draw[failflow, loop below] (gf) to node[below,labeltext]{fail: regenerate} (gf);

  % Exact G4 thresholds remain in the caption and master plan so this diagram
  % stays readable at final double-column size.
\end{tikzpicture}
\end{document}
