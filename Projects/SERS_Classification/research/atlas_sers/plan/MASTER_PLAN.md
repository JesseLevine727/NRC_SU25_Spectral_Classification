...
The **P05 locked-core implementation and bounded source-fitting smoke are complete and accepted**. On 2026-09-25 the owner separately approved a **36-fit source-validation pilot**, conditional on supervisor acceptance of its implementation. The [pilot permit](contracts/p05_development_pilot.json) and [T016 assignment](delegation/P05_TASK_016_DEVELOPMENT_PILOT.md) govern this next gate. No full development or outer evaluation follows automatically from either the smoke or the pilot.
...
1. Complete independent review of the source-validation kernel (T013), context-local selector (T014), metadata ledger (T015), and bounded pilot runner (T016). The pilot uses the fixed 30–200 epochs, patience 20 and four sampled batches per epoch. It is capped at 36 executions, 28,800 updates, 120 seconds per fit, 90 minutes overall, 4 GiB allocated CUDA and 2 GiB private output. No retries, G3 decisions or held-test predictions are permitted. Review and commit the implementation before scientific execution.
2. Review the pilot's actual stopping, shared random-stream prefixes, loss availability, checkpoints, source-validation curves, cost and storage before proposing expansion. Pilot executions consume their original registered slots within the 14,940 inner-fit ceiling; accepted evidence must be reused, not refitted as extra trials. Separately authorize any larger resource budget and retain every failed or unavailable outcome. The original 17,820 later-fit ceiling is unchanged; adding the 35 actually consumed smoke executions gives 17,855 only as a combined accounting ceiling, not permission to run.
