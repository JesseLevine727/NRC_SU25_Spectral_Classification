...
Global held-domain configuration: Stage 1 selected `derivative_1` with supervised-contrastive weight 0.5 and pair-margin weight 0.25; Stage 2 selected `legacy` with 64 embedding dimensions (720,039 total parameters).
Nested outer configurations: `minimal_minmax` + `legacy32` in 3/5 folds (361,383 total parameters); `arpls_minmax` + `legacy32` in 1/5 folds (361,383 total parameters); `minimal_minmax` + `legacy64` in 1/5 folds (720,039 total parameters). Each outer fold used only its own inner-fold selection.
...
- Historical Siamese encoder parameters: 719,584. Successor parameter counts are reported per nested/global configuration above because architecture selection was fold-specific.
